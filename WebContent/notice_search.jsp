	...
	<%
		...
		Class.forName("com.mysql.jdbc.Driver");            
			...
			if (search_value.equals("search_title")){
				String sql = "select * from notice_board where TITLE like '%"+ search +"%'"; 
				pstmt = conn.prepareStatement(sql);                
				rs = pstmt.executeQuery();
			}
			
			else if (search_value.equals("search_content")){
				String sql = "select * from notice_board where MEMO like '%"+ search +"%'"; 
				pstmt = conn.prepareStatement(sql);                
				rs = pstmt.executeQuery();
			} 
			
			else {
				String sql = "select * from notice_board where USERNAME like '%"+ search +"%'"; 
				pstmt = conn.prepareStatement(sql);                
				rs = pstmt.executeQuery();
			}
%>
	...
	%>
			<tr>
				...
				<td align="center" bgcolor="white" width="379px"><a href="notice_view.jsp?idx=<%=idx%>"><%=title%></td>
